...
impl Backend for JavaBackend {
    ...
    fn generate_bindings(&self, api: &ApiSurface, config: &SkifConfig) -> anyhow::Result<Vec<GeneratedFile>> {
        ...
        let adapter_blocks = skif_adapters::generate_adapters(config, Language::Java)?;
        if !adapter_blocks.is_empty() {
            let mut adapter_content = String::new();
            writeln!(adapter_content, "// DO NOT EDIT - auto-generated by skif").ok();
            writeln!(adapter_content, "package {};", package).ok();
            writeln!(adapter_content).ok();
            writeln!(adapter_content, "import java.lang.foreign.*;").ok();
            writeln!(adapter_content, "import java.util.*;").ok();
            writeln!(adapter_content).ok();
            writeln!(adapter_content, "public final class {}Adapters {{", main_class).ok();
            writeln!(adapter_content, "    private {}Adapters() {{}}", main_class).ok();
            for block in &adapter_blocks {
                writeln!(adapter_content).ok();
                writeln!(adapter_content, "{}", block).ok();
            }
            writeln!(adapter_content, "}}").ok();
            files.push(GeneratedFile {
                path: base_path.join(format!("{}Adapters.java", main_class)),
