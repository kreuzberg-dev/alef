...
fn gen_record_type(package: &str, typ: &TypeDef) -> String {
    ...
    writeln!(out, "package {};", package).ok();
    writeln!(out).ok();
    // Conditional imports based on field types
    let needs_list = typ.fields.iter().any(|f| field_uses_type(&f.ty, f.optional, "Vec"));
    let needs_map = typ.fields.iter().any(|f| field_uses_type(&f.ty, f.optional, "Map"));
    let needs_optional = typ
        .fields
        .iter()
        .any(|f| f.optional || matches!(&f.ty, skif_core::ir::TypeRef::Optional(_)));
    if needs_list {
        writeln!(out, "import java.util.List;").ok();
    }
    if needs_map {
