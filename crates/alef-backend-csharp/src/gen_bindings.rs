...
fn gen_record_type(
    ...
    for field in &typ.fields {
        ...
        // Use the language-level rename strategy (always wins over IR type-level serde_rename_all).
        // camelCase → to_lower_camel_case, snake_case/anything else → keep original snake_case.
        let json_name = match lang_rename_all {
            "camelCase" => field.name.to_lower_camel_case(),
            _ => field.name.clone(),
        };
