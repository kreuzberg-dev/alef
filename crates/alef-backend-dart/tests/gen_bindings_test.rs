...
#[test]
fn error_sealed_class_with_methods_emits_abstract_getters() {
    let methods = vec![
        MethodDef {
    ...
    assert!(
        content.contains("int get statusCode;"),
        "missing statusCode getter: {content}"
    );
    assert!(
        content.contains("bool get isTransient;"),
        "missing isTransient getter: {content}"
    );
    assert!(
        content.contains("String get errorType;"),
        "missing errorType getter: {content}"
    );
}
