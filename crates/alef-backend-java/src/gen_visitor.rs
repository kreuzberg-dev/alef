...
fn gen_visitor_bridge(package: &str, _class_name: &str) -> String {
    ...
    writeln!(out, "        var tagNamePtr = ctx.get(ValueLayout.ADDRESS, 8L);").ok();
    writeln!(
        out,
        "        String tagName = tagNamePtr.reinterpret(Long.MAX_VALUE).getString(0);"
    )
    .ok();
    writeln!(out, "        long depth = ctx.get(ValueLayout.JAVA_LONG, 16L);").ok();
    writeln!(out, "        long indexInParent = ctx.get(ValueLayout.JAVA_LONG, 24L);").ok();
    writeln!(out, "        var parentTagPtr = ctx.get(ValueLayout.ADDRESS, 32L);").ok();
    writeln!(
        out,
    ...
    .ok();
    writeln!(out, "        int isInlineRaw = ctx.get(ValueLayout.JAVA_INT, 40L);").ok();
    writeln!(
        out,
    ...
    writeln!(out, "            case VisitResult.Continue ignored -> 0;").ok();
    writeln!(out, "            case VisitResult.Skip ignored -> 1;").ok();
    writeln!(out, "            case VisitResult.PreserveHtml ignored -> 2;").ok();
