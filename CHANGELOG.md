...
- fix(rust-e2e): `bytes` args resolved as fixture file paths now use `../../test_documents/` (two levels up) — `CARGO_MANIFEST_DIR` for an e2e test crate is `e2e/<lang>/`, so reaching the repo-root `test_documents/` directory needs two parent steps, not one.
