...
- fix(go-e2e-codegen): emit `path/filepath` import in generated test files when any fixture in the file uses a `file_path` arg — the previous `file_path` runtime-resolution fix used `filepath.*` without adding the import, breaking Go compile with `undefined: filepath`.
