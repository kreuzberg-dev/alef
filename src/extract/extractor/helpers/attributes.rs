...
/// Extract `#[alef(string_shorthand(variant = "...", field = "..."))]` (also matching
/// `#[cfg_attr(..., alef(string_shorthand(variant = "...", field = "...")))]`).
///
/// Opt-in mapping that lets a bare host-language string construct a data-carrying enum
/// variant by routing the string into one named field. Both `variant` and `field` are
/// required; if either is missing the attribute is ignored (returns `None`).
pub(crate) fn extract_string_shorthand(attrs: &[syn::Attribute]) -> Option<crate::core::ir::StringShorthand> {
    // Parse the inner `string_shorthand(variant = "...", field = "...")` list, given a
    // `meta` positioned at the `string_shorthand` path.
    fn parse_inner(meta: &syn::meta::ParseNestedMeta<'_>) -> (Option<String>, Option<String>) {
        let mut variant = None;
        let mut field = None;
        let _ = meta.parse_nested_meta(|inner| {
            if inner.path.is_ident("variant") {
                if let Ok(v) = inner.value() {
                    if let Ok(s) = v.parse::<syn::LitStr>() {
                        variant = Some(s.value());
                    }
                }
            } else if inner.path.is_ident("field") {
                if let Ok(v) = inner.value() {
                    if let Ok(s) = v.parse::<syn::LitStr>() {
                        field = Some(s.value());
                    }
    ...
    attrs.iter().find_map(|attr| {
        let (mut variant, mut field) = (None, None);
        if attr.path().is_ident("alef") {
            let _ = attr.parse_nested_meta(|meta| {
                if meta.path.is_ident("string_shorthand") {
                    (variant, field) = parse_inner(&meta);
                } else if let Ok(v) = meta.value() {
                    let _: syn::Expr = v.parse()?;
                }
                Ok(())
            ...
            let _ = attr.parse_nested_meta(|meta| {
                if meta.path.is_ident("alef") {
                    let _ = meta.parse_nested_meta(|inner| {
                        if inner.path.is_ident("string_shorthand") {
                            (variant, field) = parse_inner(&inner);
                        } else if let Ok(v) = inner.value() {
                            let _: syn::Expr = v.parse()?;
                        }
                        Ok(())
                    ...
                    // Compound cfg predicate (e.g., `all(...)`, `any(...)`): consume inner tokens.
                    let _ = meta.parse_nested_meta(|_| Ok(()));
                }
                Ok(())
            });
        ...
        match (variant, field) {
            (Some(variant), Some(field)) => Some(crate::core::ir::StringShorthand { variant, field }),
            _ => None,
        }
    })
}
