...
/// Compute cfg attributes for functions based on their referenced parameter and return types.
///
/// When a function takes parameters or returns types that are cfg-gated, the function itself
/// must be cfg-gated with the union of those cfgs. This prevents FFI bindings from generating
/// function declarations that reference non-existent types when the feature is not enabled.
///
/// Example: If `DownloadManager` is `#[cfg(feature = "download")]`, then:
/// ```ignore
...
/// pub fn download_manager_installed_languages(client: &DownloadManager) -> Vec<String> { ... }
/// ```
pub(super) fn compute_cfg_from_referenced_types_for_functions(surface: &mut ApiSurface) {
    // Build a map of type name -> cfg for quick lookup
    let type_cfgs: AHashMap<String, String> = surface
        .types
        .iter()
    ...
    // Collect all cfg conditions referenced by each function through its params and return type
    let mut func_cfg_unions: AHashMap<String, Vec<String>> = AHashMap::new();

    for func in &surface.functions {
        // Only compute cfg unions for functions that don't already have an explicit cfg
        if func.cfg.is_some() {
            continue;
        }
        ...
        // Collect cfgs from all parameters
        for param in &func.params {
            collect_referenced_type_cfgs(&param.ty, &type_cfgs, &enum_cfgs, &mut referenced_cfgs);
        }

        // Collect cfgs from return type
        collect_referenced_type_cfgs(&func.return_type, &type_cfgs, &enum_cfgs, &mut referenced_cfgs);

        if !referenced_cfgs.is_empty() {
            func_cfg_unions.insert(func.name.clone(), referenced_cfgs);
        }
    }

    // Apply computed cfg unions back to the functions
    for func in &mut surface.functions {
        if let Some(cfgs) = func_cfg_unions.get(&func.name) {
            let union_cfg = if cfgs.len() == 1 {
                cfgs[0].clone()
            } else {
                // Multiple cfgs -> create a union: any(cfg1, cfg2, ...)
