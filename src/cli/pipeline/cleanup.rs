...
/// matches the alef-specific string `by alef` (case-sensitive, as part of
/// "auto-generated by alef" / "Generated by alef") inside the first ten lines.
/// The `by alef` substring is unique to alef's own header templates — cgo,
/// swig, autoconf, bindgen, protobuf, and other code generators all credit a
/// different tool name, so they cannot match.
pub(crate) fn has_alef_self_referential_header(path: &Path) -> bool {
    ...
    content
        .lines()
        .take(10)
        .any(|line| line.contains("by alef") || line.contains("Generated by alef"))
}
