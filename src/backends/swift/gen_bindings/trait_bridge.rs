...
pub fn gen_bridge_registration_overloads_file(
    ...
    content.push_str("    }\n");
    content.push_str("    let candidates = [pathOrContent] + roots.map { ($0 as NSString).appendingPathComponent(pathOrContent) }\n");
    content.push_str("    for path in candidates {\n");
    content.push_str("        if fm.fileExists(atPath: path), let data = try? Data(contentsOf: URL(fileURLWithPath: path)) {\n");
        ...
        for method in &trait_def.methods {
            ...
            if method.is_async {
                ...
                match &method.return_type {
                    TypeRef::Unit => {
                        content.push_str(&format!(
                            "    func {method_camel}({params}) {{}}\n\n"
                        ));
                    }
                    TypeRef::String => {
                        content.push_str(&format!(
                            "    func {method_camel}({params}) -> String {{ \"\" }}\n\n"
                        ));
                    }
                    TypeRef::Bytes => {
                        content.push_str(&format!(
                            "    func {method_camel}({params}) -> [UInt8] {{ [] }}\n\n"
                        ));
                    }
                    TypeRef::Primitive(crate::core::ir::PrimitiveType::Bool) => {
                        content.push_str(&format!(
                            "    func {method_camel}({params}) -> Bool {{ false }}\n\n"
                        ));
                    }
                    _ => {
                        // For other types, emit a placeholder comment
                        content.push_str(&format!(
                            "    // TODO: Implement {method_camel} stub\n\n"
                        ));
                    }
                }
            }
